...
class Diffusion_DEM(tf.keras.Model):
    ...
    def get_all_losses(self, Xint, Wint, Yint, XbndDir_l, WbndDir, YbndDir_l, XinitDir, YinitDir, WinitDir, Xbnd_r,
        ...
        if cycle_number == 0:
            f_val_diffusion = xPhys ** 3 * dcdy_val - xPhys ** 3 * d2cdx2_val - xPhys ** 2 * dcdx_val * 2
        else:
            ...
            f_val_diffusion = xPhys ** 3 * dcdy_val - xPhys ** 3 * d2cdx2_val - xPhys ** 2 * dcdx_val * 2 + theta * c_val * (
                    4 * xPhys ** 2 * d2udx2_val - alfa * xPhys ** 2 * dcdx_val * 2 + xPhys ** 3 * d3udx3_val - alfa * xPhys ** 3 * d2cdx2_val) + theta * xPhys * dcdx_val * (
                                      xPhys ** 2 * d2udx2_val + xPhys * dudx_val * 2 - u_val * 2 - alfa * dcdx_val * xPhys ** 2)  # stress limitations
        ...
        if cycle_number == 0:
            ...
            f_val_l_d = tf.reduce_mean(tf.math.square(xPhys_bnd_l ** 2 * dcdx_val_l - theta * c_val_bnd_l * (
                    xPhys_bnd_l ** 2 * d2udx2_val_l + xPhys_bnd_l * dudx_val_l * 2 - u_val_bnd_l * 2 - alfa * dcdx_val_l * xPhys_bnd_l ** 2)))
        ...
        if cycle_number == 0:
            ...
            f_val_r_d = tf.reduce_mean(tf.math.square(xPhys_bnd_r ** 2 * dcdx_val_r - theta * c_val_bnd_r * (
                    xPhys_bnd_r ** 2 * d2udx2_val_r + xPhys_bnd_r * dudx_val_r * 2 - u_val_bnd_r * 2 - alfa * dcdx_val_r * xPhys_bnd_r ** 2) - xPhys_bnd_r ** 2))
...
class Diffusion_DEM_Stress(tf.keras.Model):
    ...
    def get_all_losses(self, Xint, Wint, Yint, XbndDir_l, WbndDir, YbndDir_l, XinitDir, YinitDir, WinitDir, Xbnd_r,
        ...
        eps_zz_val = tf.divide(u_val, xPhys)
        ...
        stress_xx_val = lam_1 * (lam_2 * eps_xx_val + lam_3 * eps_yy_val + lam_3 * eps_zz_val)
        stress_yy_val = lam_1 * (lam_3 * eps_xx_val + lam_2 * eps_yy_val + lam_3 * eps_yy_val)
        stress_zz_val = lam_1 * (lam_3 * eps_xx_val + lam_3 * eps_yy_val + lam_2 * eps_yy_val)
        '''
        ...
        '''
        f_val_stress = tf.reduce_sum(2*((0.5*(eps_xx_val * stress_xx_val + eps_yy_val * stress_yy_val + eps_zz_val * stress_zz_val))-lam_1*alfa*c_val/3*(eps_xx_val+eps_yy_val+eps_zz_val)) * Wint)  # Energy Method
        ...
        stress_xx_val_l_s = lam_1 * (lam_2 * eps_xx_val_l_s + lam_3 * eps_yy_val_l_s * 2 - alfa * c_val_bnd_l / 3)
        ...
        stress_xx_val_r_s = lam_1 * (lam_2 * eps_xx_val_r_s + lam_3 * eps_yy_val_r_s * 2 - alfa * c_val_bnd_r / 3)
