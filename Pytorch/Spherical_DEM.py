...
class Diffusion:
    ...
    def loss_diffusion(self):
        ...
        if self.cycle_number == 1:
            loss_diffusion_pde = self.criterion(self.X_Int[:,0] * dc_dt,self.X_Int[:,0] * dc_dxx + dc_dx * 2)
        else:
            loss_diffusion_pde = self.criterion(self.X_Int[:,0]**3 * dc_dt + theta * dc_dx * (self.X_Int[:,0]**3 * dis_dxx + self.X_Int[:,0]**2 * dis_dx * 2 - self.X_Int[:,0] * dis * 2 - self.X_Int[:,0]**3 * alfa * dc_dx) + theta * c * (self.X_Int[:,0]**3 * dis_dxxx + 4 * self.X_Int[:,0]**2 * dis_dxx - self.X_Int[:,0]**3 * alfa * dc_dxx - self.X_Int[:,0]**2 * alfa * dc_dx * 2),
                                             self.X_Int[:,0]**3 * dc_dxx + self.X_Int[:,0]**2 * dc_dx * 2)
        ...
        if self.cycle_number == 1:
            ...
            loss_boundary_left = self.criterion(self.XBnd_Left[:,0]**2 * dc_dx_bnd_left,
                                            theta * c_bnd_left * (self.XBnd_Left[:,0]**2 * dis_dxx_bnd_left + self.XBnd_Left[:,0] * dis_dx_bnd_left * 2 - dis_bnd_left * 2 - alfa * self.XBnd_Left[:,0]**2 * dc_dx_bnd_left))
        ...
        if self.cycle_number == 1:
            ...
            loss_boundary_right = self.criterion(self.XBnd_Right[:,0]**2 * dc_dx_bnd_right,
                                                theta * c_bnd_right * (self.XBnd_Right[:,0]**2 * dis_dxx_bnd_right + self.XBnd_Right[:,0] * dis_dx_bnd_right * 2 - dis_bnd_right * 2- alfa * self.XBnd_Right[:,0]**2 * dc_dx_bnd_right) + self.XBnd_Right[:,0]**2)
    ...
    def loss_physics(self):
        ...
        energy_density = 0.5 * (self.X_Int[:, 0] * dis_dx * lam_1 * (lam_2 * dis_dx * self.X_Int[:, 0] + 2 * lam_3 * dis) + 2 * dis * lam_1 * (lam_3 * self.X_Int[:, 0] * dis_dx + lam_2 * dis + lam_3 * dis)) - self.X_Int[:, 0] * lam_1 * alfa * c / 3.0 * (self.X_Int[:, 0] * dis_dx + 2 * dis)
        ...
        dis_bnd_left = self.model_physics(self.XBnd_Left)[:, 0]
        dis_dx_bnd_left = torch.autograd.grad(inputs=self.XBnd_Left,outputs=dis_bnd_left,grad_outputs=torch.ones_like(dis_bnd_left),retain_graph=True,create_graph=True)[0][:, 0]
        loss_boundary_physics_left = self.criterion(lam_2*self.XBnd_Left[:,0]*dis_dx_bnd_left + 2*lam_3*dis_bnd_left, alfa*self.XBnd_Left[:,0]*c_bnd_left/3)

        c_bnd_right = self.model_diffusion(self.XBnd_Right)[:, 0]
        # boundary displacement predict
        dis_bnd_right = self.model_physics(self.XBnd_Right)[:, 0]
        dis_dx_bnd_right = torch.autograd.grad(inputs=self.XBnd_Right,outputs=dis_bnd_right,grad_outputs=torch.ones_like(dis_bnd_right),retain_graph=True,create_graph=True)[0][:, 0]
        loss_boundary_physics_right = self.criterion(lam_2*self.XBnd_Right[:,0]*dis_dx_bnd_right + 2*lam_3*dis_bnd_right, alfa*self.XBnd_Right[:,0]*c_bnd_right/3)
...
c_data = pd.read_csv('Comsol_solution/qiuc.csv', skiprows=8, header=None, names=['x', 'value'])
n_x = 501
...
u_data = pd.read_csv('Comsol_solution/qiuu.csv', skiprows=8, header=None, names=['x', 'value'])
